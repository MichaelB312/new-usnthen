...
async function processImageGenerationWithCharacters(jobId: string, params: any) {
  ...
  try {
    ...
    if (pageNumber === 1) {
      ...
      const imageFiles = await prepareImageFiles(references.slice(0, 1)); // For now, just use first ref
      ...
      // Prepare files - for now, we'll use the style anchor as base
      const imageFiles = await prepareImageFiles([references[0]]);
      
      // Call OpenAI with character-aware prompt
      const response = await openai.images.edit({
        model: 'gpt-image-1',
        image: imageFiles[0],
        prompt,
